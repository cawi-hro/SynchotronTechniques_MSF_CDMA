...
def forward_propagation_holo(z01,z02,thickness,delta,beta):
    """
    1) 
    ...
    print('The Fresnel number is', Fr)
    ...
    """
    mu = 4 * np.pi / (lam*1e-9) * beta
    transmission = np.exp(-mu*thickness) 
    print('Transmission is', np.round(transmission*100), '%.')
    ...
    O_ref = (O_phi + 1j * O_mu) * O_shape  # refractive, transmission function, irgendwas mit P multiplizieren -> Scattering
